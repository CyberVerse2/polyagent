...
const TokenList: React.FC<TokenListProps> = React.memo(
  ({ tokens = [], isLoading, showSmallBalances = false, walletAddress, refreshTrigger }) => {
    ...
    if (!tokens || tokens.length === 0) {
      return (
        <EmptyState
          title="No token assets found in wallet"
          description="Connect a wallet or switch networks to see balances."
        />
      );
    }
    ...
    if (!showSmallBalances && filteredTokens.length === 0 && tokens.length > 0) {
      return (
        <EmptyState
          title="Only small balances found"
          description="Click 'Show Small Balances' to view them."
        />
      );
    }
          ...
          <div
            ...
            key={
              isNativeBalance(item)
                ? `${item.networkName}-${item.symbol}`
                : item.contractAddress || `${item.symbol}-${index}`
            }
            ...
            style={{ boxShadow: '4px 4px 0px 0px #000000' }}
          >
            <div className="flex items-center space-x-2  w-full p-3 justify-between">
              {' '}
              {/* Removed justify-between from here */}
              <div className="flex items-center space-x-3 overflow-hidden flex-grow min-w-0 ">
                ...
                <Image
                  ...
                  alt={
                    isNativeBalance(item)
                      ? `ETH on ${item.networkName}`
                      : item.symbol || 'Token logo'
                  }
